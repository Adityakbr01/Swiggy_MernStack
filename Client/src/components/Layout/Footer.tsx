...
const footerLinks: FooterLink[] = [
  ...
  { id: "5", label: "how-it-works", path: "/how-it-works" },
  { id: "6", label: "privacy-policy", path: "/privacy-policy" },
  { id: "7", label: "terms-and-conditions", path: "/terms-and-conditions" },
  { id: "8", label: "faq", path: "/faq" },
];

const socialLinks = [
  {
    id: "1", icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
      <path d="M22.675 0h-21.35c-.732 0-1.325.593-1.325 1.325v21.351c0 .731.593 1.324 1.325 1.324h11.495v-9.294h-3.128v-3.622h3.128v-2.671c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.795.143v3.24l-1.918.001c-1.504 0-1.795.715-1.795 1.763v2.313h3.587l-.467 3.622h-3.12v9.293h6.116c.73 0 1.323-.593 1.323-1.325v-21.35c0-.732-.593-1.325-1.325-1.325z" />
    </svg>, href: "https://facebook.com", label: "Facebook"
    ,
    url: "https://facebook.com"
  },
  {
    id: "2", icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
      <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723 10.054 10.054 0 01-3.127 1.195 4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.937 4.937 0 004.604 3.417 9.868 9.868 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.054 0 13.999-7.496 13.999-13.986 0-.209 0-.42-.015-.63a9.936 9.936 0 002.46-2.548l-.047-.02z" />
    </svg>, href: "https://twitter.com", label: "Twitter"
    ,
    url: "https://twitter.com"
  },
  {
    id: "3", icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
    ...
    </svg>, href: "https://instagram.com", label: "Instagram",
            ...
            <div>
              ...
              <div className="flex space-x-4">
                ...
                {socialLinks?.map((link) => (
                  <Link key={link.id} to={link.url} target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-white transition-colors">
                    {link.icon}
                  </Link>
                ))}
              </div>
            ...
            <div>
              ...
              <ul className="space-y-2">
                {footerLinks?.map((link) => (
                  <li key={link.id}><Link to={link.path} className="text-gray-400 hover:text-white transition-colors">{link.label}</Link></li>
                ))}
              </ul>
            </div>
